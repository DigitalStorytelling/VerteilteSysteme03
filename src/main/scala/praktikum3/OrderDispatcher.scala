  ...
  def apply(actorReader: ActorRef[CommandReader], actorFinance: ActorRef[CommandFinance], actorStock: ActorRef[CommandStock]): Behavior[CommandOrderDispatcher] = {
    Behaviors.setup { context =>
      ...
      Behaviors.withStash(2000) { stashBuffer: StashBuffer[CommandOrderDispatcher] =>
        new OrderDispatcher(context, stashBuffer, actorReader, actorFinance, actorStock).waitForNextFinance()
      }

      /*Behaviors.withStash(2000) { stashBuffer: StashBuffer[CommandOrderDispatcher] =>
        new OrderDispatcher(context, stashBuffer, actorReader, actorFinance, actorStock).waitForNextStock()
      }*/
...
final class OrderDispatcher(context: ActorContext[OrderDispatcher.CommandOrderDispatcher],
  ...
  private def waitForNextFinance(): Behavior[CommandOrderDispatcher] = {
  ...
  private def activeFinance(next: WorkPullingProducerController.RequestNext[Finance.SaveCustomerAndPrice]): Behavior[CommandOrderDispatcher] = {
    ...
    Behaviors.receiveMessagePartial {
      ...
      case unknownMessage =>
        context.log.warn(s"Received unknown message in activeFinance() method: ${unknownMessage.getClass.getSimpleName}")
        Behaviors.same
    }
  }

  private def waitForNextStock(): Behavior[CommandOrderDispatcher] = {
